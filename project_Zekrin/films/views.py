...
def create(request):
    ...
    return render(request,"films/create.html",{"categori":categori})
...
def edit(request,id):
    try:
        ...
        if request.method == "POST":
            ...
            categori = Category.objects.all()
            return render(request, "films/edit.html", {"categori":categori,"films":film})
...
def categories():
    if Category.objects.all().count() == 0:
        Category.objects.create(title = "Аниме")
